...
import {
  getJobsiteInventory, addJobsiteItem, removeJobsiteItem, deleteJobsite,
  JobsiteInventory, MaterialItem, EquipmentItem, ToolItem, ItemType, AddItemPayload,
} from '../api/jobsites';

type RouteProps = RouteProp<RootStackParamList, 'JobsiteInventory'>;

type SelectedItem = { itemType: ItemType; id: number; label: string } | null;

export default function JobsiteInventoryScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProps>();
  const { jobsiteId, jobsiteName, jobsiteAddress } = route.params;

  const [inventory, setInventory]   = useState<JobsiteInventory | null>(null);
  ...
  // Delete jobsite confirmation modal
  ...
  const fetchInventory = useCallback(async (isRefresh = false) => {
    ...
    setError('');
    try {
      setInventory(await getJobsiteInventory(Number(jobsiteId)));
    } catch (err: any) {
  ...
  const handleAdd = async () => {
    ...
    setAddError('');
    try {
      await addJobsiteItem(Number(jobsiteId), payload);
      setAddVisible(false);
      fetchInventory();
    } catch (err: any) {
  ...
  const handleRemove = async () => {
    ...
    setRemoveError('');
    try {
      await removeJobsiteItem(Number(jobsiteId), selected.itemType, selected.id);
      setSelected(null);
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      await deleteJobsite(Number(jobsiteId));
      setDeleteVisible(false);
      navigation.goBack();
    } catch (err: any) {
      setDeleteError(err.message || 'Failed to delete jobsite.');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        >
          ...
          {(inventory?.materials ?? []).length === 0
            ...
            : inventory!.materials.map((item: MaterialItem) => (
              <TouchableOpacity
                key={`mat-${item.id}`}
              ...
              >
                ...
                <Text style={styles.itemDetail}>Qty: {item.amount}</Text>
          ...
          {(inventory?.equipment ?? []).length === 0
            ...
            : inventory!.equipment.map((item: EquipmentItem) => (
              <TouchableOpacity
                key={`equip-${item.id}`}
              ...
              >
                <Text style={styles.itemName}>{item.name}</Text>
                <Text style={styles.itemDetail}>Qty: {item.amount}{item.serialNumber ? `  |  S/N: ${item.serialNumber}` : ''}</Text>
                {item.description ? <Text style={styles.itemSub}>{item.description}</Text> : null}
              </TouchableOpacity>
            ))
          }
          ...
          {(inventory?.tools ?? []).length === 0
            ...
            : inventory!.tools.map((item: ToolItem) => (
              <TouchableOpacity
                key={`tool-${item.id}`}
        ...
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => { if (!removing) setRemoveVisible(false); }}>
          <TouchableOpacity style={styles.modalCard} activeOpacity={1} onPress={() => {}}>
            ...
            <Text style={styles.confirmText}>
              Remove <Text style={styles.bold}>{selected?.label}</Text>?
            </Text>
      ...
      <Modal visible={deleteVisible} transparent animationType="fade" onRequestClose={() => !deleting && setDeleteVisible(false)}>
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => { if (!deleting) setDeleteVisible(false); }}>
          <TouchableOpacity style={styles.modalCard} activeOpacity={1} onPress={() => {}}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Delete Jobsite</Text>
              <TouchableOpacity onPress={() => { if (!deleting) setDeleteVisible(false); }} style={styles.xButton}>
                <Text style={styles.xButtonText}>x</Text>
              </TouchableOpacity>
            </View>
            <Text style={styles.confirmText}>
              Permanently delete <Text style={styles.bold}>{jobsiteName}</Text>?{'\n'}
              All payorders and inventory will be removed.
            </Text>
